...
void Clipping::generateRandomLines() {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        try {
            originalLines.push_back(std::make_unique<Line>(points, indices));
...
void Clipping::clipLine(float x1, float y1, float x2, float y2) {
    ...
    if (accept) {
        ...
        try {
            clippedLines.push_back(std::make_unique<Line>(points, indices));
        } catch (const std::exception &e) {
